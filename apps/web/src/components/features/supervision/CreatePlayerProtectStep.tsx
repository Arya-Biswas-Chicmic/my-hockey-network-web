 ...
 */
export function CreatePlayerProtectStep({ playerName, onBack, onSubmit, isSubmitting }: Readonly<CreatePlayerProtectStepProps>) {
  const [settings, setSettings] = useState<ProtectSettings>(DEFAULT_PROTECT_SETTINGS);
  ...
  return (
    <div className="mhn-flow-container mhn-flow-form-wrapper">
      ...
      <div className="mhn-protect-section">
        ...
        <div className="mhn-protect-cards-stack">
          <label
            ...
            onClick={() => setSettings((p) => ({ ...p, visibility: 'private' }))}
          >
            <Input
              type="radio"
              name="visibility"
              checked={settings.visibility === 'private'}
              onChange={() => setSettings((p) => ({ ...p, visibility: 'private' }))}
              className="mhn-radio-input"
            />
            <div className="mhn-radio-card-text">
              <h4 className="mhn-radio-heading">Private</h4>
              <p className="mhn-radio-sub">Only approved hockey relationships can see {name}&apos;s profile.</p>
            </div>
          </label>

          <label
            className={`mhn-protect-radio-card ${settings.visibility === 'network' ? 'mhn-radio-selected' : ''}`}
            onClick={() => setSettings((p) => ({ ...p, visibility: 'network' }))}
          >
            <Input
              type="radio"
              name="visibility"
              checked={settings.visibility === 'network'}
              onChange={() => setSettings((p) => ({ ...p, visibility: 'network' }))}
              className="mhn-radio-input"
            />
            <div className="mhn-radio-card-text">
          ...
          {([
            ...
            <div key={key} className="mhn-protect-toggle-row">
              ...
              <Button
                ...
                className={`mhn-toggle-switch ${settings[key] ? 'mhn-toggle-on' : 'mhn-toggle-off'}`}
              >
      ...
      <div className="mhn-form-actions-stack">
        <Button className="mhn-btn-modal-cancel" onClick={onBack} disabled={isSubmitting}>
          Back
        </Button>
        <Button type="button" onClick={() => onSubmit(settings)} disabled={isSubmitting} className="mhn-btn-modal-submit">
          {isSubmitting ? <Spinner size="sm" color="#FFFFFF" /> : 'Create Account'}
        </Button>
      </div>
    </div>
  );
