...
import {
  ...
  POSITION_OPTIONS,
  SHOOTS_OPTIONS,
} from "@/config/profile-options";
import {
  useEditProfileForm,
  type EditProfileFormData,
} from "@/hooks/use-edit-profile-form";
...
interface EditProfileModalProps {
  ...
  ) => Promise<AuthMeResponse | void> | void;
  profileData?: Partial<EditProfileFormData> | Record<string, unknown> | null;
}

const fieldClassName =
  "h-11 w-full rounded-lg border border-auth-stroke bg-background px-3 text-sm text-foreground outline-none focus:border-primary";
...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  return (
    <div
      ...
      onClick={(event) =>
        event.target === event.currentTarget && handleAttemptClose()
      }
    >
      <div
        role="dialog"
        ...
        className="flex max-h-[90vh] w-full max-w-[527px] flex-col overflow-hidden rounded-xl border border-auth-stroke bg-auth-field text-foreground shadow-2xl"
      >
        <header className="flex items-center justify-between border-b border-auth-stroke px-6 py-5">
          ...
          <Button
            ...
            className="grid size-8 place-items-center rounded-full border border-auth-stroke text-muted-foreground hover:bg-secondary hover:text-foreground"
        ...
        >
          {saveSuccessMsg ? (
            <p
              role="status"
              className="rounded-lg border border-success-border bg-success-surface p-3 text-sm text-success-foreground"
            >
              {saveSuccessMsg}
            </p>
          ) : null}
          {submissionError ? (
            <p
              role="alert"
              className="rounded-lg border border-destructive-border bg-destructive-surface p-3 text-sm text-destructive"
            >
              {submissionError}
            </p>
          ) : null}

          <FormInput<EditProfileFormValues, "displayName">
            name="displayName"
            label="Name"
            required
            maxLength={50}
          ...
          <FormTextarea<EditProfileFormValues, "bio">
            ...
            label="Bio"
            rows={3}
            textareaClassName={`${fieldClassName} min-h-24 py-3`}
            containerClassName={fieldContainerClassName}
            errorClassName="text-xs text-destructive"
          />
          ...
          <FormInput<EditProfileFormValues, "dateOfBirth">
            name="dateOfBirth"
            label="Date of Birth"
            type="date"
            inputClassName={fieldClassName}
          ...
          {isPlayer ? (
            <>
              ...
              <FormSelect<EditProfileFormValues, "position">
                name="position"
                label="Position"
                options={positions.length ? positions : [...POSITION_OPTIONS]}
                selectClassName={fieldClassName}
                containerClassName={fieldContainerClassName}
                errorClassName="text-xs text-destructive"
              />
              <FormSelect<EditProfileFormValues, "shootsCatches">
                ...
                errorClassName="text-xs text-destructive"
              />
              <FormInput<EditProfileFormValues, "jerseyNumber">
                name="jerseyNumber"
                label="Jersey Number"
                type="text"
                inputMode="numeric"
          ...
          <FormInput<EditProfileFormValues, "city">
            name="city"
            label="City / Location"
            maxLength={50}
            inputClassName={fieldClassName}
