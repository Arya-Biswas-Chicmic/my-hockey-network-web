...
export function getGuardianRequestCode(
  request: GuardianRelationshipRequest,
): string {
  return request.code ?? request.devCode ?? request.inviteCode ?? "";
}

export function getGuardianRequestName(
  request: GuardianRelationshipRequest,
): string {
  const counterparty =
    request.counterparty ?? request.requester ?? request.child ?? request.minor;
  return (
...
export function GuardianRelationshipRequestCard({
  ...
  const avatarUrl = resolveMediaUrl(
    counterparty.avatarUrl ?? request.avatarUrl,
    "/userPlaceholder.webp",
  );
  const roleTag =
    counterparty.roleTag ??
    counterparty.primaryRole ??
    request.roleTag ??
    "PLAYER";
