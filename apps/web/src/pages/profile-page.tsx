...
export const ProfilePage: React.FC<PageProps> = ({ onNavigate, onLogout }) => {
  ...
  const activeProfile = targetProfileRes?.profile || (isOwnProfile ? user?.profile : null);

  const liveName = activeProfile?.displayName || activeProfile?.name || 'Player';
  const rawAvatar = activeProfile?.avatarUrl;
  const liveAvatar = resolveMediaUrl(rawAvatar, '/userPlaceholder.png');
  const rawCover =
    activeProfile?.coverImageUrl ||
    activeProfile?.coverUrl ||
    activeProfile?.coverImageKey;
  ...
  const rawRole =
    ...
    'PLAYER';
  const liveRole = activeProfile?.roleTag || String(rawRole);
