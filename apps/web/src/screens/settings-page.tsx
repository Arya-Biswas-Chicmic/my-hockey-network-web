...
export const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const { user } = useAuth();
  const [activeSubTab, setActiveSubTab] = useState<SettingsSubTabEnum>(
    SettingsSubTabEnum.NOTIFICATION,
  );
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (val.toLowerCase().includes("notif")) {
                    ...
                    val.toLowerCase().includes("role") ||
                    val.toLowerCase().includes("lang")
                  ) {
          ...
          <section className="mhn-settings-content-area">
              ...
              <div className="mhn-notification-settings-list">
                {notificationSettingsQuery.isLoading ? (
                  [1, 2, 3, 4, 5].map((n) => (
                    <div
                      key={n}
                      className="mhn-notification-setting-row mhn-notif-skeleton-row"
                    >
                  ...
                  filteredNotificationItems.map((item) => (
                    <div key={item.id} className="mhn-notification-setting-row">
                      <div className="mhn-notification-setting-text">
                        <h4 className="mhn-notification-item-title">
                          {item.title}
                        </h4>
                        <p className="mhn-notification-item-subtitle">
                          {item.subtitle}
                        </p>
                      </div>
                      <div className="mhn-btn-loading-flex">
                        {updatingNotifKey === item.id && (
                          <div className="mhn-spinner-mini" />
                        )}
                        <Switch
            ...
            {activeSubTab === SettingsSubTabEnum.GENERAL && (
              <div className="mhn-general-settings-view">
                <h3 className="mhn-settings-section-heading">
                ...
                {blockedUsersQuery.isLoading ? (
                  ...
                  <div className="mhn-blocked-users-grid">
                    {filteredBlockedUsers.map((blockedUser) => (
                      <div
                        key={blockedUser.id}
                        className="mhn-blocked-user-card"
                      >
                        <FallbackImage
                        ...
                        />
                        <h4 className="mhn-blocked-user-name">
                          {blockedUser.name}
                        </h4>
                        <span className="mhn-blocked-user-role">
                          {blockedUser.roleTag}
                        </span>

                        <div className="mhn-blocked-user-team-row">
                          <Image
                            src={blockedUser.teamLogo}
                            alt={blockedUser.teamName}
                            width={14}
                            height={14}
                            className="mhn-blocked-team-logo"
                          />
                          <span className="mhn-blocked-team-name">
                            {blockedUser.teamName}
                          </span>
                        </div>
